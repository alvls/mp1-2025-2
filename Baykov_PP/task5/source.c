...
int main()
{
	...
	{
		system("cls");
		printf("Enter the path to the specified directory in C format:\\...\\... : ");
		scanf_s("%s", catalog, 100);
		system("cls");
		strcat_s(catalog, 100, "\\*.*");
		if ((hFile = _findfirst(catalog, &c_file)) == -1L)
		{
			printf("No files in the selected directory\n");
			printf("Enter another path? 1 - yes/0 - no: ");
			scanf_s("%d", &answer);
			if (answer != 1)
			{
				system("cls");
		...
		else
		{
			...
			while (1)
			{
				printf("\tSelect sorting method:\n1 - bubble sort\n2 - selection sort\n3 - insertion sort\n4 - merge sort\n5 - quick sort\n6 - Shell sort\n7 - counting sort\nEnter the number of the selected sorting method: ");
				scanf_s("%d", &sort_type);
				system("cls");
				...
				case 7:
					...
					break;
				default:
					printf("Invalid sorting number entered\n");
					f = 1;
					break;
				}
				if (f == 0)
				{
					printf("How to sort files?\n1 - by ascending size\n2 - by descending size\n");
					scanf_s("%d", &answer);
					system("cls");
					if (answer == 1)
					{
					...
					printf("\nSort with another method? 1 - yes/0 - no: ");
					scanf_s("%d", &answer);
					if (answer != 1)
						break;
					else

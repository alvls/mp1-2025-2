...
{
    short answer;
    printf("Файлы нужно отсортировать по возрастанию или по убыванию? (по возрастанию ('1'), по убыванию ('2'))\t");
    scanf_s("%hi", &answer);
    while (getchar() != '\n');
    if ((answer != 1) && (answer != 2))
    {
        printf("Способ введен некорректно\n");
        return 3;
    }
    else return answer;
}
...
void main()
{
    ...
    do
    {
        printf("Введите путь, файла который нужно отсортировать:\t");
        ...
        if ((hFile = _findfirst(directory, &c_file)) != -1L)
        {
            ...
            do
            {
                printf("Введите номер метода сортировки ('пузырьком' - '1', 'выбором' - '2', 'вставками' - '3', 'слиянием' - '4', 'Хоара' - '5', 'Шелла' - '6', 'подсчетом' - '7'(только при max размере файла в 10000 байт), 'поразрядную' - '8'):\t");
                answer = 0;
                scanf_s("%hi", &answer);
                while (getchar() != '\n');
                switch (answer)
                ...
                case 7: while ((j < i) && (files[j].size < 10000)) j++;
                    if (i > j)
                    {
                        printf("Выбранный метод не применим к данной директории\n");
                        break;
                    }
                ...
                case 8: answer = increase_or_decrease();
                    ...
                    answer = 3;
                    break;
                default: printf("Метод сортировки введен неправильно\n");
                    break;
                }
                printf("Что дальше? Отсортировать файлы другим способом (введите '1'), перейти в другую директорию (введите '2') или закончить сортировку (введите '3')\t");
                scanf_s("%hi", &answer);
            } while (answer == 1);
            free(files);
            free(files2);
        ...
        {
            printf("Путь до директории введен неправильно\nХотите ли вы попробовать ввести путь до директории заново? (да - '2', нет - любой другой символ)\t");
            scanf_s("%hi", &answer);
        }
    } while (answer == 2);
